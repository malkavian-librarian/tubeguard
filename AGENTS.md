...
- Before planning new implementation work, query Honcho once for prior decisions and completed work.
- After developing anything new in a session, always save the durable conclusions to Honcho and record project-specific behavior or constraints in this repository's documentation.
...
- Automatic block changes use a durable outbox and explicit ownership. Never infer manual ownership after a sync write or remove a pre-existing manual block during cleanup.
- Run `npm.cmd test`, `npm.cmd run build`, and `npm.cmd run test:e2e` before completing changes to this feature.
